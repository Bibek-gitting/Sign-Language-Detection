...
with mp_hands.Hands(
    ...
    for action in actions:
        ...
        image_path = os.path.join('Image', action)
        ...
        if len(frame_files) < no_sequences:
            print(f"⚠️  Warning: {action} has only {len(frame_files)} images, need {no_sequences}")
        ...
        for sequence in range(min(len(frame_files), no_sequences)):
            ...
            if frame is None:
                print(f"❌ Missing: Image/{action}/{sequence+1}.png")
                continue
    ...
    #                 print(results)
                ...
                if keypoints is not None and np.sum(keypoints) != 0:
                    npy_path = os.path.join(DATA_PATH, action, str(sequence+1), f'{frame_num+1}.npy')
                    os.makedirs(os.path.dirname(npy_path), exist_ok=True)
                    np.save(npy_path, keypoints)
                else:
                    print(f"⚠️  No hand detected in {action} image {frame_files[sequence]}")
